...
export function InitAPI() {
  ...
  ipcMain.handle("convert", (_, file: string, fname: string) => {
    if (Wiki.current) {
      ...
      else writeFile(fpath, Buffer.from(file, "base64"));
    } else {
      new Notification({
        title: "不同寻常的错误！",
        body: "窗口聚焦问题！",
  ...
  ipcMain.handle("import", (_, file: FileInfo) => {
    if (file.path && Wiki.current) {
      const destDIR = join(Wiki.current.dir, "files");
  ...
  ipcMain.handle("gc", (_, fnames: string[]) => {
    if (Wiki.current) {
      const cwd = Wiki.current.dir;
      // 去除 ./files/ 前缀
      const basenames = fnames.map((fname) => basename(fname));
  ...
  ipcMain.on("confirm", (ev: IpcMainEvent, msg: string) => {
    const selected = dialog.showMessageBoxSync(
      Wiki.current?.win as BrowserWindow,
      {
        title: "等一下",
        buttons: ["当然", "不清楚"],
        cancelId: 1,
  ...
  ipcMain.on("alert", (ev: IpcMainEvent, msg: string) => {
    ev.returnValue =
      dialog.showMessageBoxSync(Wiki.current?.win as BrowserWindow, {
        title: "注意",
        message: msg,
      }) == 0;
  });
...
function deleteFile(path: string) {
  if (Wiki.current) {
    const cwd = Wiki.current.dir;
    const trash = join(Wiki.current.dir, "files", ".trash");
    if (!existsSync(trash)) {
      mkdirSync(trash);
    }
